...
export async function getFillInfo(page: Page, firstName: string, lastName: string, postalCode: string) {
...
export async function cartCheckout(page: Page, firstName: string, lastName: string, postalCode: string) {
    await page.getByTestId('shopping-cart-badge').click()
    ...
    return {
        pageUrl: page.url(),
        thankYou: await page.getByText('Thank you for your order!', { exact: true })
    }
}